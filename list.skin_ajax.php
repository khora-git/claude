...
if ($execution_time > 0.1) { // 0.1초 이상 걸린 요청 로깅
    error_log("Slow page load (list.skin_ajax.php): Time: {$execution_time}s");
}
?>
...
window.addEventListener('load', function() {
    ...
    if (window.performance) {
        ...
        if (pageLoadTime > 10000) {
            console.warn('페이지 로드 시간이 10초를 초과했습니다.');
        }
    }
});
